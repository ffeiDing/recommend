...
def loadData(cur_list):
    ...
    for i in range(len(book)):
        if idx > 4999:
            break
        if "馆际" in name[i] or "耳机" in name[i] or "教程" in name[i] or "指导" in name[i] or "引论" in name[i] or "数学分析" in name[i] or "高等代数" in name[i] or "考试" in name[i] or "报" in name[i] or "刊" in name[i] or "原理" in name[i] or "选编" in name[i] or "党" in name[i]:
            continue
        idx += 1
