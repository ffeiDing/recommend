...
import pymysql
import pickle
from sql_test import get_recommend_book, get_user_history
...
if __name__ == "__main__":
    ...
    database =  "loan" # 数据库名称
    if load_data:
        data = load_dic("data/data_v1.1.pkl")
        W = load_dic("data/W_v1.1.pkl")
    else:
        db = pymysql.connect(host=server, user=user, passwd=password, db=database)
        cur = db.cursor()
        data = loadData(cur)  # 获得数据
        save_dic("data/data_v1.1.pkl", data)
        W = similarity(data)  # 计算图书相似矩阵
        save_dic("data/W_v1.1.pkl", W)
        print("---已保存数据---")
    # user_id = "00639061"
    user_id = "1400012962" # "1606191027" # "0006171162"
    recommend_book_id_list = recommandList(data, W, user_id, 10, 40)  #推荐
